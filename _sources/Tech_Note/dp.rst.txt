...
1. 把原問題遞迴分割成許多更小的問題。（recurrence）
   ...
   1. 子問題與原問題的求解方式皆類似。（optimal sub-structure）
   2. 子問題會一而再、再而三的出現。（overlapping sub-problems）

2. 設計計算過程：

   1. 確認每個問題需要哪些子問題來計算答案。（recurrence）
   2. 確認總共有哪些問題。（state space）
   3. 把問題一一對應到表格。（lookup table）
   4. 決定問題的計算順序。（computational sequence）
   5. 確認初始值、計算範圍。（initial states / boundary）
